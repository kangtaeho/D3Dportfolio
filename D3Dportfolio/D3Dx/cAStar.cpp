...
void cAStar::Update(D3DXVECTOR3& position, float& rotY, float speed, float radius, D3DXVECTOR3* pEnemyPos)
{
	...
	if (FinalAStar.size())
	{
		if (D3DXVec3Length(&(g_pCollisionManager->getVector2(position) - g_pCollisionManager->getVector2(FinalAStar.front()))) < 0.001f)
		{
			FinalAStar.erase(FinalAStar.begin());
		}
		else
		{
...
bool cAStar::Stop(D3DXVECTOR3 position, float Range, D3DXVECTOR3* EnemyPosition, float EnemyRadius)
{
	if (!EnemyPosition)return false;
	if (D3DXVec3Length(&(position - *EnemyPosition)) < Range + EnemyRadius)
	{
		FinalAStar.clear();
		return true;
	}
